...
expect_that(parse(10), is_a("NumC"))
expect_that(parse(1234)@num, equals(1234))
expect_that(parse(-1234)@num, equals(-1234))
expect_that(parse(TRUE), is_a("TrueC"))
expect_that(parse(FALSE), is_a("FalseC"))
expect_that(parse(list('if', TRUE, 3, 4)), is_a("IfC"))
expect_that( parse(list('if', TRUE, 3, 4))@test, is_a("TrueC"))
expect_that( parse(list('if', TRUE, 3, 4))@thenBlock@num, equals(3))
expect_that( parse(list('if', TRUE, 3, 4))@elseBlock@num, equals(4))
expect_that(parse(list('+', 3, 4)), is_a("BinopC"))
expect_that(parse(list('-', 3, 4)), is_a("BinopC"))
expect_that(parse(list('*', 3, 4)), is_a("BinopC"))
expect_that(parse(list('/', 3, 4)), is_a("BinopC"))
expect_that(parse(list('eq?', 3, 4)), is_a("BinopC"))
expect_that(parse(list('func', list('x', 'y'), list('+', 2, 3))), is_a("LamC"))
expect_that(parse(list( list('func', list('x', 'y'), list('+', 2, 3)), 4, 5)), is_a("AppC"))
expect_that( parse(list('+', 3, 4))@symbol, equals('+')) 
expect_that( parse(list('+', 3, 4))@left@num, equals(3))
expect_that( parse(list('+', 3, 4))@right@num, equals(4))
expect_that( parse(list('+', list('-', 54, 4), 4))@left, is_a("BinopC"))
expect_that( parse(list('+',  4, list('-', 54, 4)))@right, is_a("BinopC"))
expect_that(parse(list('func', list('x', 'y'), list('+', 2, 3)))@body, is_a("BinopC"))
expect_that(parse(list('func', list('x', 'y'), list('+', 2, 3)))@params, is_a("list"))
